...
test_closed_explain_shows_the_haystack() {
  local home wt out status regions
  read -r home wt <<EOF
...
EOF
  ...
  # The count is asserted exactly, not merely present: it and the haystack below it
  # come from ONE walk of the brief, so a count that had drifted back to a second
  # reader (or been lost across a subshell) would report a different set of regions
  # than the body printed beside it.
  regions=$(grep -c 'fm:boilerplate start' "$home/data/explain-m1/brief.md")
  [ "$regions" -gt 0 ] || fail "explain: the generated brief carried no boilerplate regions to strip"
  assert_contains "$out" "boilerplate markers: ok, regions stripped: $regions, regions kept: 0" \
    "explain did not report the marker state from the same walk as the haystack"
...
# The fence tracking must not be able to inherit state from the injected boilerplate
# ABOVE the task section. The engineering conventions are copied out of the captain's
# own data/captain.md by a cut at the next heading, so a fenced example in them that
# contains a heading is truncated mid-fence, and any unbalanced fence the captain
...
test_unfilled_brief_survives_conventions_fence() {
  ...
  read -r home wt <<EOF
$(spawnable_home unfilled-conventions)
EOF
  while IFS='|' read -r label conventions; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    id="convfence-$n"
    printf '%b\n' "# Captain\n\n## Engineering conventions\n$conventions" > "$home/data/captain.md"
    FM_ROOT_OVERRIDE='' FM_HOME="$home" FM_DATA_OVERRIDE="$home/data" \
      FM_STATE_OVERRIDE="$home/state" "$BRIEF_SH" "$id" alpha >/dev/null 2>&1
    assert_grep '{TASK}' "$home/data/$id/brief.md" "$label: the fixture brief was not left unfilled"
    out=$(run_spawnable "$home" "$wt" "$id" projects/alpha codex --why captain)
    status=$?
    expect_code 4 "$status" "$label: an unfilled brief spawned anyway (got: $out)"
...
ROWS
  ...
  # having simply started refusing everything: a filled task section still spawns, and
  # a placeholder demonstrated inside a fence in the TASK body is still task text.
  FM_ROOT_OVERRIDE='' FM_HOME="$home" FM_DATA_OVERRIDE="$home/data" \
    FM_STATE_OVERRIDE="$home/state" "$BRIEF_SH" convfence-ok alpha >/dev/null 2>&1
  perl -0pi -e 's/\{TASK\}/Add a settings screen for notification preferences./' \
    "$home/data/convfence-ok/brief.md"
  out=$(run_spawnable "$home" "$wt" convfence-ok projects/alpha codex --why captain)
  status=$?
  expect_code 0 "$status" "control: a filled brief under the same conventions was refused (got: $out)"
  assert_present "$home/state/convfence-ok.meta" "control: a filled brief did not spawn"
  ...
  out=$(run_spawnable "$home" "$wt" convfence-fenced projects/alpha codex --why captain)
  status=$?
  expect_code 0 "$status" "control: a fenced placeholder demo in the task body was refused (got: $out)"
...
# closed-topic haystack's keep-more rule and deliberately so: a false refusal is loud
# and has --allow-unfilled-task, while a missed unfilled brief is silent and takes
# gate 2 down with it. A fence opened in the task body and never closed is exactly
# that uncertain case.
test_unfilled_brief_unbalanced_task_fence_refuses() {
  local home wt out status
  read -r home wt <<EOF
$(spawnable_home unfilled-openfence)
EOF
  write_brief "$home" openfence-u1 '# Task\nShow the scaffold shape:\n\n```markdown\n{TASK}\n'
  out=$(run_spawnable "$home" "$wt" openfence-u1 projects/alpha codex --why captain)
  status=$?
  expect_code 4 "$status" "an unclosed fence hid the placeholder (got: $out)"
  assert_contains "$out" "the brief was never filled in" "the refusal text changed"
  assert_absent "$home/state/openfence-u1.meta" "a refused spawn still wrote meta"
  ...
  status=$?
  expect_code 0 "$status" "control: a closed fence was read as an unfilled brief (got: $out)"
  assert_present "$home/state/openfence-u2.meta" "control: a legitimate brief did not spawn"
