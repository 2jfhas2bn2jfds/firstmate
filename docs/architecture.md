...
Every number those decisions compare - a `stat`, `date`, or `wc` result, a marker epoch, or an environment override - is coerced to an integer before the comparison, because a padded or output-buffer-polluted value aborts a bare `[ ... -ge ... ]` test and silently disables the check it guards.
An unreadable value warns loudly (an `ERROR` per occurrence in the daemon log, throttled on stderr by `FM_INT_WARN_INTERVAL_MIN`) and falls back toward acting: an unmeasurable age reads as very old, so the backstop still re-arms and the poke still fires, and the daemon truncates a marker it cannot stamp rather than persisting a fabricated epoch.
