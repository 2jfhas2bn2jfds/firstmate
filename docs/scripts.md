...
| `fm-closed-lib.sh`       | Shared closed-topic register parsing and keyword matching sourced by `fm-spawn.sh` (intake refusal) and `fm-bootstrap.sh` (session-start report); documents the `data/closed.md` line format and the exact matching rule |
