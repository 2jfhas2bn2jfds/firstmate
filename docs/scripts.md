...
| Script                   | Description                                                                                                         |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------- |
| `fm-bootstrap.sh`        | Detect required toolchain and version problems, optional capability facts, primary-checkout `TANGLE:` problems, and actionable clone refresh outcomes; refresh project clones best-effort; locally sync live secondmate homes; align the primary checkout's agent commit identity with `config/git-author`; set up opt-in X mode; ensure the always-on liveness daemon is running (inside tmux); install tools only after consent |
...
| `fm-brief.sh`            | Scaffold a ship brief with a worktree-isolation assertion, a report-only scout brief with `--scout`, or a secondmate charter with `--secondmate`; every injected block is wrapped in `fm:boilerplate` markers so the closed-topic gate can strip generated text without guessing at heading boundaries |
...
| `fm-closed-lib.sh`       | Shared closed-topic register parsing and keyword matching sourced by `fm-spawn.sh` (intake refusal) and `fm-bootstrap.sh` (session-start report); documents the `data/closed.md` line format and the exact matching rule; builds the haystack by stripping the regions `fm-brief.sh` wrapped in explicit `fm:boilerplate` markers, keeping the whole brief (loudly, when markers are unbalanced) rather than ever narrowing on heading text, and reports `- ` lines that are not well-formed entries |
