...
BRANCH="fm/$ID"
if ! fm_git -C "$WT" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null; then
  BRANCH=$(fm_git -C "$WT" symbolic-ref --quiet --short HEAD 2>/dev/null || true)
  [ -n "$BRANCH" ] || { echo "error: branch fm/$ID does not exist and worktree $WT is detached" >&2; exit 1; }
  fm_git -C "$WT" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null || { echo "error: branch $BRANCH does not exist in $WT" >&2; exit 1; }
fi
...
if fm_git -C "$PROJ" remote get-url origin >/dev/null 2>&1; then
  ...
  fm_git -C "$WT" fetch origin "+refs/heads/$DEFAULT:refs/remotes/origin/$DEFAULT" --quiet
  BASE="origin/$DEFAULT"
else
  BASE="$DEFAULT"
fi

fm_git -C "$WT" rev-parse --verify --quiet "$BASE^{commit}" >/dev/null || { echo "error: base $BASE does not exist in $WT" >&2; exit 1; }
fm_git -C "$WT" rev-parse --verify --quiet "$BRANCH^{commit}" >/dev/null || { echo "error: branch $BRANCH does not resolve in $WT" >&2; exit 1; }

echo "diff base: $BASE"
if fm_git -C "$WT" diff --quiet "$BASE...$BRANCH" --; then
  echo "no changes vs $BASE"
  exit 0
fi

fm_git -C "$WT" diff --stat "$BASE...$BRANCH" --
if ! "$STAT_ONLY"; then
  echo
  fm_git -C "$WT" diff "$BASE...$BRANCH" --
fi
