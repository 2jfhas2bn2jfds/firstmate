...
# A line that does not start with "- " and carry at least two ':' separators is
# ignored, so a hand-written note in the file never becomes a silent gate.
#
# MATCHING RULE (stated here because a gate whose rule is not written down is a
# gate nobody can predict):
#   1. Both the haystack (task id + brief text) and each keyword are normalized:
