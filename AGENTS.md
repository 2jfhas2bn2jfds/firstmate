...
A ship or scout spawn also refuses (exit 4) while its brief still carries the scaffold's `{TASK}` placeholder: an unfilled brief leaves the closure check nothing to match, so fill the brief before spawning.
