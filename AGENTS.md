...
Because the daemon is the backstop, `fm-guard.sh`'s no-watcher banner also prompts you to start it (`nohup bin/fm-supervise-daemon.sh >/dev/null 2>&1 &`) when it is not running.
