...
A ship or scout spawn also refuses (exit 4) while its brief still carries the scaffold's `{TASK}` placeholder on a line of its own, outside any fenced code block: an unfilled brief leaves the closure check nothing to match, so fill the brief before spawning. A task body that mentions the placeholder in prose, or demonstrates the scaffold's shape inside a fence, spawns normally, because the task body is free text and a brief about the brief scaffold does both. `--allow-unfilled-task` waives that check, warns loudly, and records `allowed_unfilled_task=1` in meta; like `--reopen-closed` it is a per-task waiver, so batch dispatch refuses it.
