...
By the same mechanism, and in the same place, the script sets repo-local `core.hooksPath` in that launch target when the repo carries a committed hooks directory (`.githooks`), so a project's committed git hooks apply automatically instead of depending on a remembered per-clone setup step.
That step is needed because committed hooks are reachable only through `core.hooksPath`, while `.git/hooks` is not version-controlled and dies with the pool - and crew pools are created and destroyed constantly here, so that loss is the routine case rather than a rare one.
**This makes `fm-spawn` load-bearing for those hooks: if it ever stops setting `core.hooksPath`, every project's committed hooks silently stop applying to every crew worktree**, with no error anywhere, because an absent hook is not a git failure - it is simply nothing happening.
The write is conditional precisely because `core.hooksPath` overrides `.git/hooks` entirely and `fm-spawn` launches into every project, not only the ones adopting committed hooks: it writes only when the hooks directory is both tracked in that worktree's index and checked out non-empty, and it preserves-and-reports rather than overwriting either an explicitly-set `core.hooksPath` in any scope or an existing active hook in `.git/hooks` (`*.sample` files are git's inert scaffolding and do not count).
The value written is relative, never absolute: `git config --local` from a linked worktree resolves to the pool's shared common config, so a relative path lets each checkout run its own committed hooks while a checkout whose branch lacks the directory simply runs none, whereas an absolute path would pin the whole pool to one crew worktree's copy and break when that worktree is torn down.
Because that write is pool-wide it does not only mean "each checkout runs its own committed hooks": it also reaches the `projects/<name>` primary checkout, which firstmate operates automatically and unattended, so committed hooks would otherwise become project code executing on a schedule inside the session that holds the fleet's credentials and `.env`.
**Firstmate's own automated git operations must never execute repo-committed code**, so every git call firstmate automation makes runs through the shared `fm_git` helper (`bin/fm-git-contain-lib.sh`), which disables hooks for that invocation; the containment ships with the `core.hooksPath` write rather than after it, and `tests/fm-git-contain.test.sh` fails on any new uncontained call site so the class stays closed.
The one operation `fm_git` refuses instead of containing is `git push`: the containment flag propagates into locally-spawned transport processes, so a contained push would silently suppress the remote's server-side hooks, the local no-mistakes gate's own `post-receive` included; a push whose server-side hooks matter uses plain `git` from a call site that has decided that explicitly.
The one honest gap: treehouse fires `post-checkout` from inside its own `git worktree add`, which a helper in `bin/` cannot reach.
Before any of that, firstmate's own repo-local `core.hooksPath` in the launch target is validated rather than short-circuited past: if it names a directory that does not exist, the stale value is named on stderr, because git pointed at a missing hooks directory runs no committed hook and no check gate for that project with no signal at all, and the short-circuit that would hide it is the same one that stops it ever being re-evaluated.
That validation is scoped to firstmate's own repo-local value and never to an effective one from any scope, because a project using husky legitimately sets `core.hooksPath` to a gitignored directory and warning about it every spawn would call a correctly-configured project broken and suggest a remedy that breaks it.
A skip that would disable committed hooks says so: an unresolvable git common dir (git older than 2.31, which lacks `rev-parse --path-format`) fails closed with a warning rather than silently, and the active-`.git/hooks` warning names every entry it found, so a block caused by a stray file is diagnosable instead of a mystery someone works around.
Like the identity write it is advisory and never fatal, and `FM_HOOKS_DIR` overrides the directory name (see `bin/fm-hooks-path-lib.sh`).
Every launch is prefixed with an `env -u` that strips the whole model-selection family (`ANTHROPIC_MODEL`, `ANTHROPIC_DEFAULT_OPUS_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`, `ANTHROPIC_SMALL_FAST_MODEL`, `CLAUDE_CODE_SUBAGENT_MODEL`), so each agent resolves its model from its own harness config instead of inheriting a stale pin: a tmux pane inherits its session environment, so a model id recorded there once would otherwise reach every agent launched in that session forever, including brand-new crewmates a pinned secondmate spawns; setting `FM_KEEP_MODEL_ENV` truthy skips the strip.
Stripping the whole family matters because the `ANTHROPIC_DEFAULT_*_MODEL` variables re-point the very `opus`/`sonnet`/`haiku` aliases the harness config falls back on.
A hand-typed relaunch or resume into an existing pane needs the same prefix (`stuck-crewmate-recovery`, `harness-adapters`).
...
Every generated brief (ship, scout, and the secondmate charter) also has the captain's `## Engineering conventions` section from `data/captain.md` injected near the top under a `# Engineering conventions (follow these)` heading, so those conventions reach every crewmate and secondmate the moment it spawns; the scaffold sources them at generation time (never hardcoded) and it is a graceful no-op when `data/captain.md` or that section is absent.
